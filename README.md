...
- Unit of analysis: county-level geographic unit identified by `county_fips`
- One row per county in final analytical dataset
- Geography scope default: 50 states + DC (`us_50_dc`)
...
- Default scope: ACS county universe restricted to 50 states + DC (`--geography-scope us_50_dc`)
...
python src/data/02_build_county_dataset.py --year <YEAR> --qcew data/raw/qcew_county.csv --ipeds data/raw/ipeds_institutions.csv --metro data/raw/metro_crosswalk.csv --geography-scope us_50_dc
...
- Any instructor-required override to default geography scope (`us_50_dc`)
