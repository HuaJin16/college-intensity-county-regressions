...
Default geography scope in `02_build_county_dataset.py` is `us_50_dc` (50 states + DC). Non-ACS-source county FIPS outside that ACS universe are excluded before merge and logged.
...
```bash
python src/data/02_build_county_dataset.py --year 2024 --qcew data/raw/qcew_county.csv --ipeds data/raw/ipeds_institutions.csv --metro data/raw/metro_crosswalk.csv --acs-out data/raw/acs_county_2024.csv --output data/processed/county_analysis_2024.csv --geography-scope us_50_dc
```
